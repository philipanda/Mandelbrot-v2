...
#undef main
int main() {
  printf("Main starts, initializing SDL\n");
  ...
  SDL_Window *window = SDL_CreateWindow(
      "Test", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, 0);
  ...
  int t = 0;
  int lastFrameTime = 0;
  //printf("Attempting SDL createTextureArray\n");
  createTextureArray(renderWidth, renderHeight);
  //printf("SDL created texture array!\n");
  while (!quit) {
    //printf("\ntick!\n");
    ...
    if (screenshot) {
      ...
      do {
        try {
          ...
          shot = SDL_CreateRGBSurface(0, w, h, 32, 0xFF000000, 0x00FF0000,
    ...
    if ((pressed[Right] - pressed[Left]) || (pressed[Down] - pressed[Up]) ||
        (pressed[In] - pressed[Out]) || (pressed[Plus] - pressed[Minus]) ||
        (pressed[ResUp] - pressed[ResDown]))
      dirty = true;

    if (dirty) {
      dirty = false;
      Complex d = (end - start) * moveSpeed;
      start += Complex(d.real * (pressed[Right] - pressed[Left]),
                       d.imaginary * (pressed[Down] - pressed[Up]));
      end += Complex(d.real * (pressed[Right] - pressed[Left]),
                     d.imaginary * (pressed[Down] - pressed[Up]));
      ...
      int newRes =
          renderWidth + renderWidth * 0.1 * (pressed[ResUp] - pressed[ResDown]);
      renderWidth = newRes > WIDTH ? WIDTH : (newRes > 10 ? newRes : 10);

      newRes = renderHeight +
               renderHeight * 0.1 * (pressed[ResUp] - pressed[ResDown]);
    ...
    t = SDL_GetTicks();
    //fflush(stdout);
    int slowDownCowboy = (1000 / 60) - lastFrameTime;
    slowDownCowboy = slowDownCowboy > 0 ? slowDownCowboy : 0;
